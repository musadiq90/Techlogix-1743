CREATE OR REPLACE FUNCTION OFSFTP.TLX_RT_TXN_CSTM (BATCHID    VARCHAR2,
                                                   MISDATE    VARCHAR2)
   RETURN NUMBER
AS
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE ('STARTING TLX_RT_TXN_CSTM');

   SELECT COUNT (1)
     INTO L_TOTAL
     FROM FTP_STTM_RT_TXN_CUSTOM
    WHERE STATUS IN ('U') AND BATCH_ID = LV_BATCH_ID;
   ...
   LOOP
      IF NOT FN_CREATE_RT_TXN (REC.XREF, REC.STATUS)
      THEN
         L_ERROR := L_ERROR + 1;
      ELSE
...
EXCEPTION
   ...
   THEN
      ...
      DBMS_OUTPUT.PUT_LINE ('TLX_RT_TXN_CSTM FAILED');
      RETURN 0;
END;
/
