CREATE OR REPLACE FUNCTION OFSFTP.FTP_TLX_RV_TXN_CSTM (BATCHID    VARCHAR2,
                                                       MISDATE    VARCHAR2)
   RETURN NUMBER
AS
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE ('STARTING FTP_TLX_RV_TXN_CSTM');

   SELECT COUNT (1)
     INTO L_TOTAL
     FROM FTP_STTM_RT_TXN_CUSTOM
    WHERE STATUS IN ('AA', 'CE') AND BATCH_ID = LV_BATCH_ID AND FIC_MIS_DATE = MIS_DATE;
   ...
   FOR REC
      IN (SELECT XREF, STATUS
            FROM FTP_STTM_RT_TXN_CUSTOM
           WHERE     STATUS IN ('AA', 'CE')
                 AND FIC_MIS_DATE = MIS_DATE
                 AND BATCH_ID = LV_BATCH_ID)
   LOOP
      IF NOT FTP_FN_REVERSE_RT_TXN (REC.XREF, REC.STATUS)
      THEN
   ...
   DBMS_OUTPUT.PUT_LINE ('ENDING FTP_TLX_RV_TXN_CSTM');
   RETURN 1;
EXCEPTION
   WHEN OTHERS
   THEN
      ...
      DBMS_OUTPUT.PUT_LINE ('FTP_TLX_RV_TXN_CSTM FAILED');
      RETURN 0;
END;
/
